...
class ItemControllerTest {
	...
	public void before() {
	    ...
	    itemDto = new ItemDto(1, "Ball", 10, 100);
	}
	...
	void testGetItemsBasic() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.get("/items/all-items-from-database").accept(MediaType.APPLICATION_JSON);
		mockMvc.perform(request)
							.andExpect(status().isOk())
	...
	void testCreateItem() throws Exception {
		...
		when(itemService.createItem(Mockito.any())).thenReturn(itemDto);
		RequestBuilder request = MockMvcRequestBuilders.post("/items/item")
		...
		mockMvc.perform(request)
							...
							.andExpect(header()
									.string("location", containsString("/item/")))
							.andReturn();
	}
}
